...
import com.india.springdemo.model.GraphData;
...
public class CoronaDailyStatusService {
	...
	private List<GraphData> allStats = new ArrayList<>();
	...
	private void fetchCoronaViruisData() throws IOException, InterruptedException, URISyntaxException {
		...
		List<Integer> dailyReport = new ArrayList<>();
			...
			for (CSVRecord record : records) {
				...
				if (country.equals("India")) {
					...
					for (Entry entry : map.entrySet()) {
						graph.add(new GraphData(entry.getKey().toString(), entry.getValue().toString()));
					}
				}
			}
			this.allStats = graph;
	 ...
	 * @return the allStats
	 */
	public List<GraphData> getGraphStatus() {
		return allStats;
	}
